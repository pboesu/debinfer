...
From the traceplot we can see that the burnin period is about 250 samples. We can remove the burnin and have a look at parameter correlations using `pairs.debinfer_result`.
```{r}
burnin = 250
...
```
Overlay the true underlying parameter values as vertical dashed lines shows that our estimation is doing a pretty good job.
