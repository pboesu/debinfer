...
## WALB model with logistic food (see Teixeira et al. 2014 J Se Res). Juvenile phase only
...
walb_deb_log_food <- function(t, states, params){
  with(as.list(c(states, params)), {
    ...
    log_df_n <- function(t, lAsym=f_lAsym, uAsym=f_uAsym, rate = f_rate, xmid= f_xmid){
...
setparams.DEB.walb_logfood<-function(L_m = 26.62883424,
