...
#' @param samp.type character; type of sampler: "rw" = Normal random walk, "ind" = independence, "rw-unif" = asymmetric uniform distribution, "rw-ref01" = reflecting random walk sampler with bounds [0,1] (cf. Hoff 2009, Chapter 10.5.1)
#'
...
debinfer_par <- function(name, var.type, fixed, value, joint=NULL, prior=NULL, hypers=NULL, prop.var=NULL, samp.type=NULL){
  ...
  if(!fixed) if(!samp.type %in% c("rw", "rw-unif","ind", "rw-ref01")) stop('samp.type must be one of c("rw", "rw-unif","ind", "rw-ref01")')
  ...
  if(!fixed) if(samp.type == "rw-ref01") if(!is.numeric(prop.var) | prop.var < 0 | length(prop.var)!=1) stop("prop.var must be a numeric > 0 of length 1 for sampler type 'rw-ref01'")
  if(!fixed) if(samp.type == "rw-ref01") if(prop.var >= 1) warning("prop.var should be << 1 for efficient sampling with sampler type 'rw-ref01'")
